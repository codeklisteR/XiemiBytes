    ...
    if (!AppState.isLoggedIn()) {
        app.innerHTML = `
            <div class="fade-in container mt-4" style="max-width: 480px; margin: 100px auto; text-align: center;">
                ...
                <a href="#/login" class="btn btn-primary px-5 py-2" style="border-radius:10px; font-weight:200;">Sign In to Continue</a>
            </div>
        `;
        return;
    }
        ...
        if (!cart || cart.items.length === 0) {
            app.innerHTML = `
                <div class="fade-in container mt-4" style="max-width: 520px; margin: 80px auto; text-align: center;">
                    ...
                    <a href="#/products" class="btn btn-primary px-5 py-2">Browse Menu</a>
                </div>
            `;
            return;
        }
        ...
        app.innerHTML = `
            <div class="cart-page fade-in container mt-4" style="max-width: 1000px;">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h1 class="page-section-title mb-1">Cart</h1>
                        <p class="text-muted small mb-0">${cart.items.length} item(s)</p>
                    </div>
                ...
                <div class="cart-layout" style="display: flex; gap: 28px; flex-wrap: wrap;">
                    ...
                    <div class="cart-items" style="flex: 2; min-width: 300px;">
                        ${cart.items.map(item => `
                            <div class="cart-item" style="display: flex; gap: 16px; padding: 16px; margin-bottom: 12px; align-items: center; border-radius: 8px; border: 1px solid var(--color-border); background: white;">
                                ...
                                <div style="flex: 1; min-width: 0;">
                                    ...
                                    <div class="mt-1" style="color: var(--color-primary); font-size: 0.9rem;">${Utils.formatCurrency(item.unitPrice || item.product.price)} each</div>
                                </div>
                                ...
                                <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 10px; flex-shrink: 0;">
                                    ...
                                    <button class="btn-cart-remove" data-item-id="${item.id}" style="font-size: 0.75rem; color: #ef4444; background: none; border: none; cursor: pointer; padding: 0;">Remove</button>
                                </div>
                            </div>
                        `).join('')}
                    </div>
                    ...
                    <div style="flex: 1; min-width: 280px;">
                        <div class="profile-panel" style="position: sticky; top: 80px;">
                            ...
                            <div class="d-flex justify-content-between mb-2">
                                ...
                                <span class="font-weight-regular">${Utils.formatCurrency(subtotal)}</span>
                            </div>
                            ...
                            <a href="#/products" class="btn btn-light btn-block mt-2" style="border-radius:12px;">
                                Continue Shopping
                            </a>
                        </div>
                    </div>
