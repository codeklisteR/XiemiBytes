...
class AppStateManager {
    ...
    persistState(changes) {
        if ('user' in changes) {
            if (this.user) Storage.setUser(this.user);
            else Storage.removeUser();
        }
        if ('token' in changes) {
            if (this.token) Storage.setToken(this.token);
    ...
    isAdmin() {
        ...
        return role && [CONSTANTS.USER_ROLE.MANAGER, CONSTANTS.USER_ROLE.SUPERADMIN].includes(role);
    }
    ...
    isStaff() {
        const role = this.user?.role?.toLowerCase();
        return role && [CONSTANTS.USER_ROLE.STAFF, CONSTANTS.USER_ROLE.MANAGER, CONSTANTS.USER_ROLE.SUPERADMIN].includes(role);
    }
     ...
     */
    isSuperAdmin() {
        return this.user && this.user.role?.toLowerCase() === CONSTANTS.USER_ROLE.SUPERADMIN;
    }
